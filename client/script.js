                        ...
                        .then(anchorResponseData => {
                            ...
                            anchorTable.innerHTML += `
                                <tr>
                                    ...
                                    <td class="anchor-link-status status-${anchorStatus}">${anchorStatus}</td>
                                </tr>
                            `;
                        });
                    });
...
function crawlManualInput() {
    const pageTitle = document.getElementById('pageTitle').value;
    const pageLink = document.getElementById('pageLink').value;

    // Simulate JSON-like data for manual input
    const manualData = {
        'page-list': [
            {
                'Title': pageTitle,
    ...
    const pages = manualData['page-list'];
    pages.forEach(page => {
        const pageLink = page['Link'];
        fetch(`https://server-response-50018810936.catalystappsail.in/server-response?page-link=${pageLink}`)
        .then(response => response.json())
